...
import type { AdminResourceChangeSummary, AdminResourceMoveTarget, AdminResourceRow, AdminResourceTreeValidation } from '@/pages/AdminResourceManagement/types'
 ...
 * 新增／更新／位置變動。
 */
export function useAdminResourceTree() {
  const logger = useLogger({ prefix: 'AdminResourceTree', enabled: import.meta.env.DEV })
  ...
  /** 上次讀取時的快照，用來算出未儲存變更 */
  const originalRows = ref<AdminResourceRow[]>([])
  ...
  const changeSummary = computed<AdminResourceChangeSummary>(() => {
  ...
  /** 把後端的嵌套樹攤平成扁平陣列 */
  function flattenNodes(nodes: AdminResourceTreeNodeResponse[], parentRowKey: string | null, level: number): AdminResourceRow[] {
    const result: AdminResourceRow[] = []
  ...
  /** 依同層順序取出子節點 */
  function childrenOf(parentRowKey: string | null): AdminResourceRow[] {
    return rows.value.filter((row) => row.parentRowKey === parentRowKey).sort((a, b) => a.position - b.position)
  }

  /** 以前端識別碼查節點 */
  function findRow(rowKey: string): AdminResourceRow | null {
    return rows.value.find((row) => row.rowKey === rowKey) ?? null
  }

  /** 全部子孫（不含自己） */
  function descendantsOf(rowKey: string): AdminResourceRow[] {
    const result: AdminResourceRow[] = []
  ...
  /** 完整資源名稱（含父路徑），送出與顯示共用 */
  function fullPathOf(row: AdminResourceRow): string {
   ...
   */
  function visibleRows(expandedRowKeys: Set<string> | null): AdminResourceRow[] {
    const result: AdminResourceRow[] = []
   ...
   */
  function addRow(parentRowKey: string | null): AdminResourceRow | null {
   ...
   * @param rowKey 要搬移的節點
   */
  function moveTargetsFor(rowKey: string): AdminResourceMoveTarget[] {
    const row = findRow(rowKey)
    if (row === null) {
      return []
    }
    ...
    const targets: AdminResourceMoveTarget[] = []
  ...
  function validateTree(): AdminResourceTreeValidation {
