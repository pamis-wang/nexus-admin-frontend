...
/** 表格顯示用的列，含呈現需要的衍生資訊 */
export interface AdminResourceTableRow {
  /** 對應的樹節點 */
...
/** 拖曳結果：把某節點放到同層某節點之後 */
export interface AdminResourceDragMove {
  /** 被拖曳的節點 */
  rowKey: string
...
/** 儲存前的整棵樹驗證結果 */
export interface AdminResourceTreeValidation {
  /** 是否通過驗證 */
  isValid: boolean
  /** 未通過時的第一個錯誤訊息 */
  message: string | null
...
/** 未儲存變更摘要 */
export interface AdminResourceChangeSummary {
  /** 新增筆數 */
  addedCount: number
  /** 改名或啟用狀態變動的筆數 */
  updatedCount: number
...
/** 「移到其他層級」對話框的候選父節點 */
export interface AdminResourceMoveTarget {
