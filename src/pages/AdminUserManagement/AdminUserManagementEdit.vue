    ...
    <q-card-section>
      ...
      <q-form class="q-gutter-md" @submit="handleSubmit" @reset="handleReset">
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-md-6">
            <AdminUserRoleSelector v-model="formData.roleIds" @load-failed="handleRoleLoadFailed" />
          </div>
        </div>
...
import type { AdminUserManagementEditFormData } from '@/pages/AdminUserManagement/types'

const route = useRoute()
const router = useRouter()
const dialog = useDialog()
const notify = useNotify()
const logger = useLogger({ prefix: 'AdminUserManagementEdit', enabled: import.meta.env.DEV })

/** 電子信箱格式，與後端的 [EmailAddress] 一致地只做基本檢查 */
const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

const formData = reactive<AdminUserManagementEditFormData>({ id: '', account: '', email: '', fullName: '', roleIds: [], isDisabled: false })
const isSystemDefault = ref(false)
const isLoadingUser = ref(true)
const isSubmitting = ref(false)
/** 載入當下的內容，按「重設」時還原 */
const originalFormData = ref<AdminUserManagementEditFormData | null>(null)
