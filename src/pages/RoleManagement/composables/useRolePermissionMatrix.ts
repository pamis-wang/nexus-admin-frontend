...
import type { RolePermissionResource, RolePermissionState, RolePermissionTreeNode, RolePermissionType } from '@/pages/RoleManagement/types'
...
export function useRolePermissionMatrix() {
  const logger = useLogger({ prefix: 'RolePermissionMatrix', enabled: import.meta.env.DEV })

  /** 扁平的全部資源，依 displayOrder 排序 */
  const resources = ref<RolePermissionResource[]>([])
  /** 每個資源目前的權限狀態，key 為 resourceId */
  const permissions = reactive<Record<string, RolePermissionState>>({})
  /** 載入當下的權限快照，供「重設」還原 */
  const originalPermissions = ref<Record<string, RolePermissionState>>({})
  const isLoadingResources = ref(false)
  const isLoadingPermissions = ref(false)

  const isLoading = computed(() => isLoadingResources.value || isLoadingPermissions.value)

  const treeNodes = computed<RolePermissionTreeNode[]>(() => buildTreeNodes(null))
  ...
  function buildTreeNodes(parentResourceId: string | null): RolePermissionTreeNode[] {
  ...
  /** 直接子資源 */
  function childrenOf(resourceId: string): RolePermissionResource[] {
  ...
  /** 同層的其他資源 */
  function siblingsOf(resourceId: string): RolePermissionResource[] {
   ...
   */
  function hasPermission(resourceId: string, permissionType: RolePermissionType): boolean {
   ...
   * @param value 勾選或取消
   */
  function updatePermission(resourceId: string, permissionType: RolePermissionType, value: boolean) {
  ...
  function clearDependentPermissions(state: RolePermissionState) {
  ...
  /** 依同層的勾選結果回頭調整父節點 */
  function syncParentPermission(resourceId: string, permissionType: RolePermissionType, value: boolean) {
    const parentResourceId = parentIdOf(resourceId)
  ...
  /** 目前權限狀態的深層複製 */
  function snapshotPermissions(): Record<string, RolePermissionState> {
    const snapshot: Record<string, RolePermissionState> = {}
