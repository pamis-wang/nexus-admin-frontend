...
import RoleManagementList from '@/pages/RoleManagement/RoleManagementList.vue'
...
/** 掛載列表頁並等待資料載入 */
async function mountRoleManagementList() {
  ...
  const wrapper = mount(RoleManagementList, { global: { plugins: [quasarPlugin, customPlugin, router] } })
...
describe('RoleManagementList', () => {
  ...
  it('載入後渲染角色列，系統預設角色標上徽章', async () => {
    const wrapper = await mountRoleManagementList()
